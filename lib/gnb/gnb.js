...
import { refreshToken } from "@/lib/api/AuthService"; //삭제용
...
const GlobalNavigationBar = ({ isLoggedin, setIsLoggedIn, points, nickname, handleAuthChange, accessToken }) => {
  ...
  const handleRefreshToken = async () => { //삭제용
    try {
  ...
  return (
    <div className={styles.globalNavigationBar_container}>
      {isLoggedin ? (
        <Link href="/pocketPlace" className={styles.homeLogo} onClick={() => console.log("Logo clicked")}>
          <Image
            src="/assets/logo.svg"
            width={100}
            height={100}
      ...
      <div className={styles.menu_container}>
        ...
        {isLoggedin ? (
          <>
            <button onClick={handleRefreshToken}>쿠키용</button>
            {timeLeft === 0 && (
              <Image
